...
public:
    ...
    // 물약 이름으로 레시피를 검색하는 메서드
    void searchRecipeByName(const std::string& name) const {
        bool found = false; // 검색 결과가 있는지 확인하기 위한 깃발(Flag)

        for (size_t i = 0; i < recipes.size(); ++i) {
            // 입력받은 이름과 저장된 레시피의 이름이 일치하는지 비교
            if (recipes[i].potionName == name) {
                ...
                for (size_t j = 0; j < recipes[i].ingredients.size(); ++j) {
                    std::cout << recipes[i].ingredients[j] << (j < recipes[i].ingredients.size() - 1 ? ", " : "");
                }
                std::cout << "\n---------------------------" << std::endl;

                found = true;
                break; // 찾았으니 더 이상 반복문을 돌 필요가 없음
            }
        }
